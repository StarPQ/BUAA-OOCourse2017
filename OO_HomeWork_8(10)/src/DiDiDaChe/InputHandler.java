...
public class InputHandler extends Thread{
	//Overview: read lines from System.in judge whether the input is valid and not same to others
	//不变式：this.MapreqList!=null&&this.reqList!=null&&this.MaYun!=null
	//&&this.MapreqList.size()<5&&this.reqList[i]is String for all 0<i<this.reqList.size&&this.reqList[i] != this.reqList[j] for all 0<=i <j<=this.reqList.size
	//&&this.MapreqList[i]is String for all 0<i<this.MapreqList.size&&this.MapreqList[i] != this.MapreqList[j] for all 0<=i <j<=this.MapreqList.size
	public boolean repOK() {
		if(this.MapreqList==null||this.reqList==null||this.MaYun==null) return false;
		if(this.MapreqList.size()>5)return false;
		for(int i=0;i<reqList.size();i++){
			if (!(reqList.get(i) instanceof String))return false;
			for(int j=i+1; j<reqList.size();j++)
				if(reqList.get(i).equals(reqList.get(j)))return false;
		}
		for(int i=0;i<MapreqList.size();i++){
			Object x = MapreqList.get(i);
			if (!(x instanceof String))return false;
			for(int j=i+1; j<MapreqList.size();j++)
				if(MapreqList.get(i).equals(MapreqList.get(j)))return false;
		}
		return true;
	}
	public InputHandler(JackMa MaYun) {
		...
		@ EFFECTS: 	initialize reqList		初始化请求队列，
					initialize MapreqList	初始化地图改变请求队列
		*/
		this.MaYun = MaYun;
	...
	public void run() {
		/* 
		@ REQUIRES: None
		@ MODIFIES: reqList MapreqLst, System.out
		@ EFFECTS: 	(line matches"\\[CR,\\(\\+?0*\\d{1,2},\\+?0*\\d{1,2}\\),\\(\\+?0*\\d{1,2},\\+?0*\\d{1,2}\\)\\]")&&(0<from<79)&&(0<to<79)&&(from!=to)
						==>reqList.add&&req.start
					(line matches"\\[CM,\\(\\+?0*\\d{1,2},\\+?0*\\d{1,2}\\),\\(\\+?0*\\d{1,2},\\+?0*\\d{1,2}\\),\\+?(0+|0*1)\\]")&&(0<from<79)&&(0<to<79)
						&&((from.x==to.x&&abs(from.y-to.y)==1)||(from.y==to.y&&abs(from.x-to.x)==1))
						==>MaYun.changeMap(from,to) change map connection
					处理读入的指令，无效输出，有效调用相应函数执行，并通过指令队列检查重复
		*/
