...
public class Map {
	...
	public myPoint getNextPosforS(myPoint Pos) {
		/* 
		@ REQUIRES: None
		@ MODIFIES: None
		@ EFFECTS: 	\result this.map.getNextPos(Pos); random point that leads to minimal stream value
					返回流量最小的随机邻接点
		*/
		return Map[Pos.getX()][Pos.getY()].getNextPosforS();
	}
	...
	public LinkedList<myPoint> getChildforS(int x, int y) {
		/* 
		@ REQUIRES: p在地图内
		@ MODIFIES: None
		@ EFFECTS: 	this.Map[p.x][p.y].U==>res.add(mypoint(p.x-1,p.y))
...
///////////////////////////////////////////////////////////////////////////////////
		...
		*/
		...
		for(int i=1; i<6400;){
			...
			for(myPoint tmp: parent.getChild()){
				...
				else if(dis[tmp.getX()][tmp.getY()] > dis[parent.getX()][parent.getY()]+parent.getStreamValue(tmp)){
					dis[tmp.getX()][tmp.getY()] = dis[parent.getX()][parent.getY()]+parent.getStreamValue(tmp);
				}
			}
		}
	...
	public synchronized int[][] getMatforS(int x, int y){
		/* 
		@ REQUIRES: p(x,y)在地图范围内
		@ MODIFIES: None
		@ EFFECTS: 	normal_behaver
		...
		*/
		...
		for(int i=1; i<6400;){
			myPoint parent = Array.pollFirst();
			//System.out.println(parent.getX()+","+parent.getY()+":"+parent.getChild().size());
			for(myPoint tmp: parent.getChildforS()){
				if(visited[tmp.getX()][tmp.getY()]==false){
					visited[tmp.getX()][tmp.getY()]=true;
					Array.add(Map[tmp.getX()][tmp.getY()]);
					dis[tmp.getX()][tmp.getY()] = dis[parent.getX()][parent.getY()]+parent.getStreamValue(tmp);
...
//////////////////////////////////////////////////////////////////////////////////
	...
	public synchronized void StreamValueUP(myPoint from, myPoint to){
		/* 
		...
		@ EFFECTS: 	Map[from.x][from.y].streamValue++, Map[to.x][to.y].streamValue++
					改变地图流量
		*/
		Map[from.getX()][from.getY()].StreamValueUP(to);
		Map[to.getX()][to.getY()].StreamValueUP(from);
	}
	public synchronized void StreamValueDOWN(myPoint from, myPoint to){
		/* 
		@ REQUIRES: ((from.x==to.x&&abs(from.y-to.y)==1)||(from.y==to.y&&abs(from.x-to.x)==1)) from，to为邻接点
		@ MODIFIES: Map[from.x][from.y], Map[to.x][to.y]中对应点流量
		@ EFFECTS: 	Map[from.x][from.y].streamValue++, Map[to.x][to.y].streamValue++
					改变地图流量
		*/
		Map[from.getX()][from.getY()].StreamValueDOWN(to);
		Map[to.getX()][to.getY()].StreamValueDOWN(from);
	}
///////////////////////////////////////////////////////////////////////////////////////
